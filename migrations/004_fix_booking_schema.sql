-- Migration: Fix booking schema data types and structure
-- This migration handles the conversion of existing columns to match the new schema

-- First, let's check what columns exist and their current types
-- Then we'll convert them safely

-- Convert adults column from text to integer if it exists as text
DO $$ 
BEGIN
...
-- Convert children column from text to integer if it exists as text
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
...
-- Convert total_amount column from text to numeric if it exists as text
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
...
-- Handle travel_date column - rename check_in_date if it exists
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
...
-- Handle guest_name to contact_name conversion if needed
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
...
-- Handle guest_email to contact_email conversion if needed
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
...
-- Handle guest_phone to contact_phone conversion if needed
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
...
-- Handle guest_count to adults conversion if needed
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns 
